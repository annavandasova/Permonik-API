...
import cz.incad.nkp.inprove.permonikapi.originentities.metatitle.dto.MetaTitleWithSpecimensStatsDTO;
...
public class MetaTitleService {
    ...
    public Optional<MetaTitle> getMetaTitleById(String metaTitleId) {
        Criteria criteria = new Criteria(MetaTitleDefinition.SHOW_TO_NOT_LOGGED_USERS_FIELD).is(true).and(MetaTitleDefinition.ID_FIELD).is(metaTitleId);

        SimpleQuery simpleQuery = new SimpleQuery(criteria);
        simpleQuery.setRows(1);

        Page<MetaTitle> page = solrTemplate.queryForPage(MetaTitleDefinition.META_TITLE_CORE_NAME, simpleQuery, MetaTitle.class);
    ...
    public List<MetaTitle> getAllMetaTitles(){
        Criteria criteria = new Criteria(MetaTitleDefinition.SHOW_TO_NOT_LOGGED_USERS_FIELD).is(true);
